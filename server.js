...
const busData = JSON.parse(fs.readFileSync(dataPath, 'utf8'));
...
app.get('/api/buses', async (req, res) => {
  ...
  try {
    ...
    const busesSnapshot = await busesRef.where('route', '==', route).get();
    const reservationsRef = firestore().collection('reservations');
    const reservationsSnapshot = await reservationsRef.where('date', '==', date).get();

    const bookedSeats = [];
    reservationsSnapshot.forEach(doc => {
      const reservation = doc.data();
      if (reservation.seats) {
        bookedSeats.push(...reservation.seats);
      }
    });
    ...
    busesSnapshot.forEach(doc => {
      ...
      updatedBus.seats = bus.seats.map(seat => ({
        ...seat,
...
app.get('/api/available-seats', authenticateUser, async (req, res) => {
  const { route, date } = req.query;
  if (!route || !date) {
    return res.status(400).send('Route and date are required');
  }
  ...
  try {
    const busRef = admin.firestore().collection('buses').doc(route);
...
app.post('/api/reserve-seat', authenticateUser, (req, res) => {
  const {
    route, date, seats, passengerInfo,
  } = req.body;
