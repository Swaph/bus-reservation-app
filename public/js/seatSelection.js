...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('seatSelection.js loaded with:', { route, date, busName, departureTime, arrivalTime, busType, fare, numberPlate });
    ...
    fetch(`/api/available-seats?route=${route}&date=${date}`)
        ...
        .then(data => {
            ...
            seatsElements.forEach(seat => {
                ...
                // ✅ Manually mark special seats as unavailable
                if (seatNumber === 'Driver' || seatNumber === 'ICE' || seatNumber === '38') {
                    seat.classList.add('unavailable');
                } else {
                    // ✅ Find seat in the array
                    const seatData = bus.seats.find(s => s.seatNumber === seatNumber);

                    if (seatData) {
                        if (seatData.available === 0) {
                            seat.classList.add('booked');  // ✅ Mark booked seats
                            seat.classList.remove('selected');  // ✅ Ensure booked seats can't be selected
                        } else {
                            seat.classList.remove('booked');  // ✅ Ensure available seats are selectable
                        }
                    }
                }
...
function removeSeat(seatNumber, fare, source) {
    ...
    const rowToRemove = Array.from(selectedSeatsTable.querySelectorAll('tr')).find(row =>
        row.dataset.seat === seatNumber
    );
    if (rowToRemove) {
        rowToRemove.remove();
    }
...
function removePassengerInfo(seatNumber) {
    const passengerDiv = Array.from(passengerInfo.children).find(div =>
        div.querySelector('label').textContent.includes(`Passenger Information for Seat ${seatNumber}`)
    );
    if (passengerDiv) {
...
function bookSeats() {
    ...
    fetch('/api/book-seat', {
        ...
        body: JSON.stringify({
            route,
            date,
            busNumberPlate: numberPlate,  // ✅ Ensure `busNumberPlate` is included
            seats: Object.keys(passengerInfoData),
            passengerInfo: passengerInfoData
        })
    })
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                alert(data.error || 'Failed to book seats. Please try again.');
            }
        })
        .catch(error => {
